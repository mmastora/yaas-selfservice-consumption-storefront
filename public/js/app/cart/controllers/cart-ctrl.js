...
/**
 * This controller manages the interactions of the cart view. The controller is
 * listening to the 'cart:udpated' event and will refresh the scope's cart
 * instance when the event is received.
 */
.controller('CartCtrl', [ '$scope', '$state', '$rootScope', 'CartSvc', 'GlobalData', 'settings', 'AuthSvc', 'AuthDialogManager', 'CartREST', 'AccountSvc', function($scope, $state, $rootScope, CartSvc, GlobalData, settings, AuthSvc, AuthDialogManager, CartREST, AccountSvc) {
	...
	/**
	 * Issues an "update cart" call to the service or removes the item if the
	 * quantity is undefined or zero.
	 */
			...
			dlg.then(function() {
				...
				if (AuthSvc.isAuthenticated()) {
					...
					AccountSvc.account().then(function(account) {
						CartREST.Cart.one('carts', null).get({
							customerId : GlobalData.customerAccount.id,
							siteCode : GlobalData.getSiteCode()
						}).then(function(authUserCart) {
							// there is an existing cart - update scope instance
							var cart = authUserCart.plain();
							CartSvc.refreshCart(cart.id, 'manual');
						}, function() {
