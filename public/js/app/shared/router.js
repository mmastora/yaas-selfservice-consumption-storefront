...
.config([ '$stateProvider', '$urlRouterProvider', '$locationProvider', function($stateProvider, $urlRouterProvider, $locationProvider) {
	...
	$stateProvider.state('base', {
		...
		resolve : {
			// this will block controller loading until the application has been
			// initialized with
		...
		resolve : {
			...
			// initialized with
			// all required configuration (language, currency)
			dummy : [ 'initialized', function(initialized) {// force
															// initialization
															// delay
				if (initialized) {
					return {};
		...
		resolve : {
			product : [ '$stateParams', 'PriceProductREST', 'CategorySvc', 'initialized', function($stateParams, PriceProductREST, CategorySvc, initialized) {
				if (initialized) {
					return PriceProductREST.ProductDetails.one('productdetails', $stateParams.productId).customGET('', {
						expand : 'media'
					}).then(function(prod) {
			...
			variants : [ '$stateParams', 'initialized', '$http', 'SiteConfigSvc', function($stateParams, initialized, $http, SiteConfigSvc) {
				if (initialized) {
					// $http used since 'option' property in response body is
					// not handled correctly by Restangular
			...
			variantPrices : [ '$stateParams', 'initialized', '$http', 'SiteConfigSvc', 'GlobalData', function($stateParams, initialized, $http, SiteConfigSvc, GlobalData) {
				if (initialized) {
	...
	.state('base.checkout.details', {
		...
		resolve : {
			...
			orders : [ 'OrderListSvc', function(OrderListSvc) {
				...
				return OrderListSvc.query(parms).then(function(response) {
					return response;
				}, function() {
					return [];
				});
		...
		views : {
			'main@' : {
				templateUrl : 'js/app/account/templates/selfservice-order-detail.html',
				controller : 'SelfserviceAccountOrderDetailCtrl'
			}
		},
		resolve : {
			order : [ '$stateParams', 'OrdersREST', function($stateParams, OrdersREST) {
				return OrdersREST.Orders.one('orders', $stateParams.orderId).get().then(function(result) {
					window.scrollTo(0, 0);
	...
	.state('base.support', {
		...
		views : {
			'main@' : {
				templateUrl : 'js/app/support/templates/support.html',
				controller : 'SupportCtrl'
			}
		},
		resolve : {
			// this will block
			// controller loading until
	...
	/* Support for Text Chat & Video Chat */
	.state('base.support.chat', {
		url : '/chat',
		views : {
			'main@' : {
				templateUrl : 'js/app/support/templates/support-chat.html',
				controller : 'SupportCtrl'
			}
		},
		resolve : {
	...
	// support tickets and registrations
	.state('base.support.tickets', {
		url : '/mytickets',

		views : {
			'main@' : {
				templateUrl : 'js/app/support/templates/MySupportTickets.html',
				controller : 'MySupportTicketsCtrl'
			}
		},
		resolve : {
			// this will block
			...
			// configuration (language,
			// currency)
			supportTickets : [ 'SupportSvc', 'initialized',function(SupportSvc,initialized) {
				if (initialized){
					var parms = {
						// q:"'status:OPEN'",
						// pageNumber:1,
					...
					return SupportSvc.getSupportTickets(parms).then(function(response) {
						// debugger;
						return response;
					}, function() {
						return [];
	...
	// support prod registrations

	// view prod regs
	.state('base.support.registrations', {
		url : '/myregistrations',

		views : {
			'main@' : {
				templateUrl : 'js/app/support/templates/MySupportRegistrations.html',
				controller : 'MySupportRegsCtrl'
			}
		},
		resolve : {
			// this will block
			...
			// configuration (language,
			// currency)
			productRegistrations : [ 'SupportSvc', 'GlobalData', 'initialized', function(SupportSvc, GlobalData, initialized) {
				if (initialized) {
					var parms = {
						pageSize : 100,
						sort : 'createdAt:desc'
					};
					return SupportSvc.getRegisteredProducts(parms).then(function(response) {
						return response;
					}, function() {
						return [];
					});
	...
	.state('base.myorders', {
		...
		views : {
			'main@' : {
				templateUrl : 'js/app/support/templates/MyOrders.html',
	...
	//chat transcripts
	// support tickets and registrations
	.state('base.support.mychattranscripts', {
		url : '/mychattranscripts',

		views : {
			'main@' : {
				templateUrl : 'js/app/support/templates/MyChatTranscripts.html',
				controller : 'MyChatTranscriptsCtrl'
			}
		},
		resolve : {
			// this will block
			...
			// configuration (language,
			// currency)
			chatTranscripts : [ 'SupportSvc', 'initialized','GlobalData',function(SupportSvc,initialized,GlobalData) {
				if (initialized){
					/*
					var parms = {
					...
					return SupportSvc.getChatTranscripts(parms).then(function(response) {
						// debugger;
						return response;
					}, function() {
						return [];
